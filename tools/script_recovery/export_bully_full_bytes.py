#!/usr/bin/env python3
"""Reproduce complete retail NOVI_Bully Init/Main/GivenTeddy snapshots."""
from __future__ import annotations
import argparse,hashlib,json
from pathlib import Path
import pefile
from capstone import CS_ARCH_X86,CS_MODE_32,Cs
REGIONS=(("Init",0x00DAED30,0x00DAEDDC),("Main",0x00DBB310,0x00DBCCFF),("GivenTeddy",0x00DBCD00,0x00DBCD58))
def export(exe:Path)->dict:
 ...
 return {'schema':'bully-full-retail-bytes/0.1','source':'installed retail Fable.exe PE image','sourceSha256':hashlib.sha256(raw).hexdigest().upper(),'nativeClass':'CNOVI_Bully','regions':rows,'coverage':{'operations':'complete Init, Main dispatcher, and GivenTeddy helper','subsumes':['teddy dispatch','health bar','subdued run-off','GivenTeddy focused snapshots'],'boundaryCorrection':'translation_unit Main size 6630 truncates nine epilogue bytes; exact terminal ret is 0x00DBCCFE'}}
def main()->int:
