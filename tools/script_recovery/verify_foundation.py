...
def verify(root: Path) -> dict[str, Any]:
    ...
    check("evidence-complete native helpers execute as standalone Lua",
          helper_lua_manifest["summary"] == {"emitted": 17, "deploymentEligible": 0} and
          {row["targetAddress"] for row in helper_lua_entries} == {
              ...
              "0x007E7410", "0x00CBED82", "0x00F14250", "0x00F14270", "0x00F25840"} and
          all(not row["deploymentEligible"] for row in helper_lua_entries) and
          helper_lua_validation["summary"] == {
              "helpers": 17, "passed": 17, "checks": 113, "complete": True},
    ...
    check("generated helpers reduce the native conversion backlog",
          readiness_summary["resolvedNativeHelperMethods"] == 17 and
          readiness_summary["resolvedNativeHelperCalls"] == 191 and
          readiness_summary["scriptsWithResolvedNativeHelpers"] == 127 and
          readiness_summary["unresolvedNativeHelperMethods"] == 136 and
          readiness_summary["unresolvedNativeHelperCalls"] == 1386 and
