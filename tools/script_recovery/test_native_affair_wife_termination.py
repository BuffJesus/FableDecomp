...
from tools.script_recovery import test_native_affair_wife_acquisition as fixtures
from tools.script_recovery.native_affair_wife_acquisition import recover_affair_wife_acquisition
...
class AffairWifeTerminationTests(unittest.TestCase):
    def inputs(self):
        fn, source, data, manifest = fixtures.AffairWifeAcquisitionTests().inputs()
        source, _ = recover_affair_wife_acquisition(fn, source, data)
    ...
    def test_loop_enters_when_live_and_stops_at_next_saved_result(self):
        ...
        result, evidence = recover_affair_wife_termination(fn, source, data)
        self.assertEqual(evidence[0]['status'], 'recovered')
        self.assertNotIn('cVar5 = extraout_AL_00;', result)
        self.assertNotIn('cVar5 = extraout_AL_22;', result)
