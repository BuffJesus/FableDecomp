...
from tools.script_recovery import test_native_affair_wife_cleanup as fixtures
from tools.script_recovery.native_book_trader_cleanup import recover_affair_wife_cleanup
...
class AffairWifeArgumentHitTests(unittest.TestCase):
    def inputs(self):
        fn, source, data, manifest = fixtures.AffairWifeCleanupTests().inputs()
        source, _ = recover_affair_wife_cleanup(fn, source, data)
    ...
    def test_hit_classification_truth_table_and_query_order(self):
        ...
        result, evidence = recover_affair_wife_argument_hits(fn, source, data, manifest)
        self.assertEqual(evidence[0]['status'], 'recovered')
        self.assertNotIn('LAB_00db3762', result)
        ...
        # Saved-register aliases remain a separate recovery; do not erase them.
        self.assertEqual(lifter.todo, ['unlifted: unaff_EBP = uVar14;'] * 2)
        lua = LuaRuntime()
        run = lua.execute('return function(me,uVar21)\n' + body + '\nend')
        inspect = lua.execute('return function(me,uVar21)\n' + body.replace('return bVar4',
                              'return bVar4, uVar14, cVar5') + '\nend')
    ...
    def test_changed_native_ability_rejects_recovery(self):
        ...
        def changed(address, size):
            ...
            if address == 0xDB2B10:
                raw = bytearray(raw)
                raw[0xDB374A - address] = 13
                return bytes(raw)
            return raw
        result, evidence = recover_affair_wife_argument_hits(fn, source,
