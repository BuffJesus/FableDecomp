#!/usr/bin/env python3
"""Reproduce complete retail NOVI_CreatedBeetle Init/Main byte snapshots."""
from __future__ import annotations
import argparse,hashlib,json
from pathlib import Path
import pefile
from capstone import CS_ARCH_X86,CS_MODE_32,Cs
REGIONS=(("Init",0x00CDEBB0,0x00CDEBB1),("Main",0x00DB80C0,0x00DB81A8))
def export(exe:Path)->dict:
 ...
 return {'schema':'created-beetle-full-retail-bytes/0.1','source':'installed retail Fable.exe PE image','sourceSha256':hashlib.sha256(raw).hexdigest().upper(),'nativeClass':'CNOVI_CreatedBeetle','regions':rows,'coverage':{'operations':'shared empty Init and complete Main','timerSeconds':5,'removeThingArgs':[True,True],'normalCleanup':'0x00DB8183-0x00DB8198','terminationCleanup':'0x00DB8199-0x00DB81A7'}}
def main()->int:
