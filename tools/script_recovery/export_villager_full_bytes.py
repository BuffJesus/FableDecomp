#!/usr/bin/env python3
"""Reproduce complete retail NOVI_Villager Init/Main/helper byte snapshots."""
from __future__ import annotations
import argparse,hashlib,json
from pathlib import Path
import pefile
from capstone import CS_ARCH_X86,CS_MODE_32,Cs
REGIONS=(("Init",0x00DADF00,0x00DADF52),("Main",0x00DADF80,0x00DAEA63),("GetVillagerSpeechIndex",0x00DAEBF0,0x00DAEC5D))
def export(exe:Path)->dict:
 ...
 return {'schema':'villager-full-retail-bytes/0.1','source':'installed retail Fable.exe PE image','sourceSha256':hashlib.sha256(raw).hexdigest().upper(),'nativeClass':'CNOVI_Villager','regions':rows,'coverage':{'operations':'complete Init, Main dispatcher, and speech-index helper','speechAxes':['sex','good deeds','bad deeds','both/neither','nonrepeating random index'],'hitOutcome':'AddBadDeed(2) and attacked speech'}}
def main()->int:
