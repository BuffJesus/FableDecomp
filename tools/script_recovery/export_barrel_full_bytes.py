#!/usr/bin/env python3
"""Reproduce complete retail NOVI_Barrel byte snapshots."""
from __future__ import annotations
import argparse,hashlib,json
from pathlib import Path
import pefile
from capstone import CS_ARCH_X86,CS_MODE_32,Cs
REGIONS=(("Init",0x00CDEBB0,0x00CDEBB1),("OnPredicateFail",0x00DB7DB0,0x00DB7DE1),("Main",0x00DB7E10,0x00DB7FEC))
def export(exe:Path)->dict:
 ...
 return {'schema':'barrel-full-retail-bytes/0.1','source':'installed retail Fable.exe PE image','sourceSha256':hashlib.sha256(raw).hexdigest().upper(),'nativeClass':'CNOVI_Barrel','regions':rows,'coverage':{'operations':'shared empty Init, complete OnPredicateFail, and complete Main','predicateWrites':['BarrelBrokenInstantaneous','BarrelBrokenPersistent','BarrelBrokenPos'],'terminalMain':'0x00DB7FE6-0x00DB7FEB'}}
def main()->int:
