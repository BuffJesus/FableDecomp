#!/usr/bin/env python3
"""Reproduce complete retail NOVI_Theresa Init/Main byte snapshots."""
from __future__ import annotations
import argparse,hashlib,json
from pathlib import Path
import pefile
from capstone import CS_ARCH_X86,CS_MODE_32,Cs
REGIONS=(("Init",0x00DAC4F0,0x00DAC57E),("Main",0x00DB97A0,0x00DBB305))
def export(exe:Path)->dict:
 ...
 return {'schema':'theresa-full-retail-bytes/0.1','source':'installed retail Fable.exe PE image','sourceSha256':hashlib.sha256(raw).hexdigest().upper(),'nativeClass':'CNOVI_Theresa','regions':rows,'coverage':{'operations':'complete Init and Main dispatcher','outcomes':['meet','chocolate yes/no and presentation','hit response','raid outro'],'boundaryCorrection':'translation_unit Main size 7010 truncates the epilogue; exact terminal ret is 0x00DBB304'}}
def main()->int:
