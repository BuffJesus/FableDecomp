#!/usr/bin/env python3
"""Reproduce complete retail NOVI_Victim Init/Main byte snapshots."""
from __future__ import annotations
import argparse,hashlib,json
from pathlib import Path
import pefile
from capstone import CS_ARCH_X86,CS_MODE_32,Cs
REGIONS=(("Init",0x00DAEEB0,0x00DAEF4C),("Main",0x00DBCD60,0x00DBDE35))
def export(exe:Path)->dict:
 ...
 return {'schema':'victim-full-retail-bytes/0.1','source':'installed retail Fable.exe PE image','sourceSha256':hashlib.sha256(raw).hexdigest().upper(),'nativeClass':'CNOVI_Victim','regions':rows,'coverage':{'operations':'complete Init and Main dispatcher','sharedBullyState':['BullySubdued','VictimShake','BullyRanOff','HeroAttackedVictim','GivenHeroTeddy','VictimComplainsAboutLosingTeddy'],'boundaryCorrection':'translation_unit Main size 4303 truncates the epilogue; exact terminal ret is 0x00DBDE34'}}
def main()->int:
