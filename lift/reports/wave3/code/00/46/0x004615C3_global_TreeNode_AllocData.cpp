...
void __stdcall TreeNode_AllocData(std::uint32_t* param_1)
{
    ...
    if (reinterpret_cast<std::uint32_t*>(reinterpret_cast<std::uintptr_t>(pvVar1) + 0x10) != nullptr)
    {
        auto* const allocData = static_cast<TreeNodeAllocDataOverlay*>(pvVar1);
        allocData->value_10 = param_1[0];
        allocData->value_14 = param_1[1];
    }
}