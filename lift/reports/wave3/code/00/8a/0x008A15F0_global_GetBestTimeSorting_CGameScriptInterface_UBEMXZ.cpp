...
{
public:
    float __thiscall GetBestTimeSorting() const;
};
...
struct TimeSortingSource_Overlay
{
...
static_assert(offsetof(TimeSortingSource_Overlay, flags_20) == 0x20);
static_assert(offsetof(TimeSortingSource_Overlay, interface_map) == 0x44);
static_assert(offsetof(TimeSortingSource_Overlay, flags_91) == 0x91);

struct CTCBase_Overlay
{
    std::byte pad_00[0x17C];
    float field_17C;
};
static_assert(offsetof(CTCBase_Overlay, field_17C) == 0x17C);

using Fn_00449970 = void* (__thiscall*)(void*);
using Fn_00487DE0 = TimeSortingSource_Overlay* (__thiscall*)(void*);
...
float __thiscall CGameScriptInterface::GetBestTimeSorting() const
{
    const auto* const self = reinterpret_cast<const CGameScriptInterface_Overlay*>(this);
    TimeSortingSource_Overlay* const source = CALL_00487DE0(CALL_00449970(self->field_14));
    ...
    if (source != nullptr && (source->flags_91 & 1u) == 0 && (source->flags_20 & 0x10u) != 0)
    {
        ...
        return reinterpret_cast<const CTCBase_Overlay*>(entry->value)->field_17C;
    }
