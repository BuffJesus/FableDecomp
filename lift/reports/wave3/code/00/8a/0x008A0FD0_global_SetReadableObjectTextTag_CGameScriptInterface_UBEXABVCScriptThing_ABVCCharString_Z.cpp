...
{
public:
    void __thiscall SetReadableObjectTextTag(
...
struct SetReadableObjectTextTag_TargetOverlay
{
...
static_assert(offsetof(SetReadableObjectTextTag_TargetOverlay, m_Flags20) == 0x20);
static_assert(offsetof(SetReadableObjectTextTag_TargetOverlay, m_InterfaceMap44) == 0x44);
static_assert(offsetof(SetReadableObjectTextTag_TargetOverlay, m_Flags91) == 0x91);

struct SetReadableObjectTextTag_InterfaceOverlay
{
    std::byte m_Pad00[0x18];
    CWideString m_Text18; // 0x18
};

static_assert(offsetof(SetReadableObjectTextTag_InterfaceOverlay, m_Text18) == 0x18);

using Fn_0099E430 = CWideString* (__thiscall*)(const CCharString* pThis, CWideString* pOut);

inline constexpr Fn_0099E430 CALL_0099E430 =
    reinterpret_cast<Fn_0099E430>(0x0099E430);
}

void __thiscall CGameScriptInterface::SetReadableObjectTextTag(
    const CScriptThing& scriptThing,
    const CCharString& textTag) const
{
    const auto* const pScriptThing = reinterpret_cast<const CScriptThingOverlay*>(&scriptThing);
    auto* const pTarget = reinterpret_cast<SetReadableObjectTextTag_TargetOverlay*>(
        ...
        ((pTarget->m_Flags20 & 0x00002000) != 0))
    {
        ...
        alignas(CWideString) std::byte wideTextStorage[sizeof(CWideString)];
        CWideString* const pWideText =
            CALL_0099E430(&textTag, reinterpret_cast<CWideString*>(wideTextStorage));

        reinterpret_cast<SetReadableObjectTextTag_InterfaceOverlay*>(pEntry->m_Value)->m_Text18 =
            *pWideText;

        pWideText->~CWideString();
    }
}