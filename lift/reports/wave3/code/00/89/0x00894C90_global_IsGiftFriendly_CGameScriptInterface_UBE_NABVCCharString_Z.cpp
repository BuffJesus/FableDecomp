...
{
public:
    bool __thiscall IsGiftFriendly(const CCharString& giftType) const;
};
...
bool __thiscall CGameScriptInterface::IsGiftFriendly(const CCharString& giftType) const
{
    ...
    if (foundDrunkennessDef)
    {
        ...
        if (foundWillPowerIndicator)
        {
            ...
            CRefCountedDefOverlay* const pWillPowerOverlay =
                reinterpret_cast<CRefCountedDefOverlay*>(const_cast<CAICreatureWillPowerIndicatorDef*>(willPowerIndicatorDef.m_pObject));
            ...
            {
                reinterpret_cast<void(__thiscall*)(void*)>(pWillPowerOverlay->m_pVftable[1])(willPowerIndicatorDef.m_pObject);
            }
