...
{
public:
    long __thiscall GetRoughExperienceLevel() const;
};

class CGameScriptInterface
{
public:
    long __thiscall GetHeroRoughExperienceLevel() const;
};
...
struct CGameScriptInterface_GetHeroRoughExperienceLevel_SelfOverlay
{
...
static_assert(
    offsetof(
        CGameScriptInterface_GetHeroRoughExperienceLevel_SelfOverlay,
        m_Field14) == 0x14);

struct CGameScriptInterface_GetHeroRoughExperienceLevel_TCInterfaceEntryOverlay
{
...
static_assert(
    offsetof(
        CGameScriptInterface_GetHeroRoughExperienceLevel_TCInterfaceEntryOverlay,
        m_Key) == 0x00);
static_assert(
    offsetof(
        CGameScriptInterface_GetHeroRoughExperienceLevel_TCInterfaceEntryOverlay,
        m_pInterface) == 0x04);
static_assert(
    sizeof(CGameScriptInterface_GetHeroRoughExperienceLevel_TCInterfaceEntryOverlay) == 0x08);

struct CGameScriptInterface_GetHeroRoughExperienceLevel_TargetOverlay
{
...
static_assert(
    offsetof(
        CGameScriptInterface_GetHeroRoughExperienceLevel_TargetOverlay,
        m_Flags2C) == 0x2C);
static_assert(
    offsetof(
        CGameScriptInterface_GetHeroRoughExperienceLevel_TargetOverlay,
...
using Fn_00487DD1 =
    CGameScriptInterface_GetHeroRoughExperienceLevel_TargetOverlay* (__thiscall*)(void*);
...
long __thiscall CGameScriptInterface::GetHeroRoughExperienceLevel() const
{
    const auto* const self =
        reinterpret_cast<
            const CGameScriptInterface_GetHeroRoughExperienceLevel_SelfOverlay*>(this);
    auto* const target =
        CALL_00487DD1(CALL_00449970(self->m_Field14));
    ...
    if ((target != nullptr) && ((target->m_Flags91 & 0x01) == 0))
    {
        ...
        if (pHeroExperience != nullptr)
        {
            return pHeroExperience->GetRoughExperienceLevel();
        }
    }
