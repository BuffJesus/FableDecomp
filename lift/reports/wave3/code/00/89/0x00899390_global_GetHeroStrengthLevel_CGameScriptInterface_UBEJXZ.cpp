...
{
public:
    long __thiscall GetHeroStrengthLevel() const;
};
...
struct CGameScriptInterface_GetHeroStrengthLevel_SelfOverlay
{
...
static_assert(
    offsetof(
        CGameScriptInterface_GetHeroStrengthLevel_SelfOverlay,
        m_Field14) == 0x14);

struct CGameScriptInterface_GetHeroStrengthLevel_TCInterfaceEntryOverlay
{
...
static_assert(
    offsetof(
        CGameScriptInterface_GetHeroStrengthLevel_TCInterfaceEntryOverlay,
        m_Key) == 0x00);
static_assert(
    offsetof(
        CGameScriptInterface_GetHeroStrengthLevel_TCInterfaceEntryOverlay,
        m_pInterface) == 0x04);
static_assert(
    sizeof(CGameScriptInterface_GetHeroStrengthLevel_TCInterfaceEntryOverlay) == 0x08);

struct CGameScriptInterface_GetHeroStrengthLevel_TargetOverlay
{
...
static_assert(
    offsetof(
        CGameScriptInterface_GetHeroStrengthLevel_TargetOverlay,
        m_Flags2C) == 0x2C);
static_assert(
    offsetof(
        CGameScriptInterface_GetHeroStrengthLevel_TargetOverlay,
...
using Fn_00487DC0 =
    CGameScriptInterface_GetHeroStrengthLevel_TargetOverlay* (__thiscall*)(void*);
...
long __thiscall CGameScriptInterface::GetHeroStrengthLevel() const
{
    const auto* const self =
        reinterpret_cast<
            const CGameScriptInterface_GetHeroStrengthLevel_SelfOverlay*>(this);
    auto* const target =
    ...
    if ((target != nullptr) && ((target->m_Flags91 & 0x01) == 0))
    {
        ...
        return pHeroExperience->GetTotalStatLevel(0);
    }
