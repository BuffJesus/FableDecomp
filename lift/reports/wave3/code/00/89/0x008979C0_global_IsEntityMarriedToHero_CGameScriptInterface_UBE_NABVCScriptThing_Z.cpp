...
{
public:
    bool __thiscall IsMarriedToThePlayer();
};

class CGameScriptInterface
{
public:
    bool __thiscall IsEntityMarriedToHero(const CScriptThing& entity) const;
};
...
bool __thiscall CGameScriptInterface::IsEntityMarriedToHero(const CScriptThing& entity) const
{
        ...
        ((pThing->m_Flags38 & 0x00800000) != 0))
    {
        ...
        if (reinterpret_cast<CTCWife*>(pWifeEntry->m_Value)->IsMarriedToThePlayer())
        {
            return true;
        }
    }
