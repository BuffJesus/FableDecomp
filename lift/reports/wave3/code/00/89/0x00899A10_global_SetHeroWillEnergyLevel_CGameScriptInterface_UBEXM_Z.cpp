...
{
public:
    void __thiscall SetHeroWillEnergyLevel(float willEnergyLevel) const;
};
...
struct CGameScriptInterface_SetHeroWillEnergyLevel_SelfOverlay
{
...
static_assert(
    offsetof(
        CGameScriptInterface_SetHeroWillEnergyLevel_SelfOverlay,
        m_Field14) == 0x14);

struct CGameScriptInterface_SetHeroWillEnergyLevel_TCInterfaceEntryOverlay
{
...
static_assert(
    offsetof(
        CGameScriptInterface_SetHeroWillEnergyLevel_TCInterfaceEntryOverlay,
        m_Key) == 0x00);
static_assert(
    offsetof(
        CGameScriptInterface_SetHeroWillEnergyLevel_TCInterfaceEntryOverlay,
...
struct CGameScriptInterface_SetHeroWillEnergyLevel_TargetOverlay
{
...
static_assert(
    offsetof(
        CGameScriptInterface_SetHeroWillEnergyLevel_TargetOverlay,
        m_Flags20) == 0x20);
static_assert(
    offsetof(
        CGameScriptInterface_SetHeroWillEnergyLevel_TargetOverlay,
...
using Fn_00487DC0 =
    CGameScriptInterface_SetHeroWillEnergyLevel_TargetOverlay* (__thiscall*)(void*);
...
void __thiscall CGameScriptInterface::SetHeroWillEnergyLevel(float willEnergyLevel) const
{
    const auto* const self =
        reinterpret_cast<
            const CGameScriptInterface_SetHeroWillEnergyLevel_SelfOverlay*>(this);
    auto* const target =
    ...
    if ((target != nullptr) && ((target->m_Flags91 & 0x01) == 0))
    {
        if ((target->m_Flags20 & 0x10) != 0)
        {
            ...
            reinterpret_cast<CTCHeroStats*>(pEntry->m_pInterface)
                ->ForceStaminaTo(willEnergyLevel);
            return;
        }

        auto* const pHeroStats =
            reinterpret_cast<CTCHeroStats*>(
                static_cast<std::uintptr_t>(std::bit_cast<std::uint32_t>(willEnergyLevel)));
        pHeroStats->ForceStaminaTo(willEnergyLevel);
    }
}