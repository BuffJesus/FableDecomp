# Controller enums — PROVEN from debug_build/FableWin.pdb (2026-08-09)

Extracted via llvm-pdbutil `pretty --enums` (DIA). These SUPERSEDE the inferred
values in CONTROLLER_ENUMS.md. crc0/name-keyed; used by the redefine/gamepad patch.

## EControllerType (the device-type tag; record +0x04)
```
enum EControllerType {
CONTROLLER_NONE = 0
CONTROLLER_XBOX_PAD = 1
CONTROLLER_KEYBOARD = 2
CONTROLLER_MOUSE = 3
}
```

## EXboxControllerButton (record +0x0C when ControllerType==1)
CORRECTS the inferred face-button values (was A=1/B=2/X=3/Y=4; PROVEN below):
```
...
XBOX_PAD_LEFT_ANALOGUE_STICK = 17
XBOX_PAD_RIGHT_ANALOGUE_STICK = 18
}
```

## EGameAction (record +0x00; 114 values)
```
enum EGameAction {
GAME_ACTION_NULL = 0
