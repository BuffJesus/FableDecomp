-- Experimental identity-preserving ForgeFSE override configuration.
--
-- Keep disabled for ordinary installs. A disposable-save playtest may set enabled
-- and allowUnverifiedDisposable to true and must supply the exact acknowledgement
-- required by ForgeFSE. The runtime then replaces only Q_NewOakValeIntro's native
-- allocator; it does not register a second quest identity.
RetailOverrides = {
    enabled = false,
