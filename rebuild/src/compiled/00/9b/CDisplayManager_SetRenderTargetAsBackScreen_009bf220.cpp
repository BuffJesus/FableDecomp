...
bool CDisplayManagerBackScreenView::SetRenderTargetAsBackScreen()
{
    renderColour0C = backColour2C;
    renderDepth1C = backDepth3C;
    renderTargetDimensions194 =
        renderColour0C.GetDimensions();
    InitVirtualPixels();

    if (displayDevice58->SetRenderTarget(
            0,
            backColour2C.GetSurface()) >= 0)
    {
        if (displayDevice58->SetDepthStencilSurface(
                backDepth3C.GetSurface()) >= 0)
        {
            C2DBoxI viewport = {
                0,
                0,
