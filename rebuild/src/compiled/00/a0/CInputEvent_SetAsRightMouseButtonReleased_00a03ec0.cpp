...
enum EInputEventType {
    INPUT_EVENT_RIGHT_MOUSE_RELEASED = 12
};
...
struct CInputEventRetail {
    ...
    void SetAsRightMouseButtonReleased(
        const C2DVector& position,
        double startTime,
        double currentTime);
};

void CInputEventRetail::SetAsRightMouseButtonReleased(
    const C2DVector& position,
    double startTime,
    double currentTime)
{
    DeviceType = INPUT_DEVICE_MOUSE;
    Type = INPUT_EVENT_RIGHT_MOUSE_RELEASED;
