...
enum EInputEventType {
    INPUT_EVENT_MIDDLE_MOUSE_RELEASED = 9
};
...
struct CInputEventRetail {
    ...
    void SetAsMiddleMouseButtonReleased(
        const C2DVector& position,
        double startTime,
        double currentTime);
};

void CInputEventRetail::SetAsMiddleMouseButtonReleased(
    const C2DVector& position,
    double startTime,
    double currentTime)
{
    DeviceType = INPUT_DEVICE_MOUSE;
    Type = INPUT_EVENT_MIDDLE_MOUSE_RELEASED;
