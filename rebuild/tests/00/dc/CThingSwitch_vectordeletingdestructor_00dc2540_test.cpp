...
{
    AutoTinyFirstMemberDestructor(static_cast<unsigned char*>(self) + 100);
    AutoTinySecondMemberDestructor(static_cast<unsigned char*>(self) + 76);
...
int main()
{
    ...
    if (CThingSwitch_CompositeScalarDeletingDestructor(object, 0, 0) != object ||
        g_AutoTinyFirstMember != object + 100 ||
        g_AutoTinySecondMember != object + 76 ||
    ...
    if (g_AutoTinySequenceCount != 6 ||
        ...
        return 1;
    std::printf("AUTO_TINY_00dc2540_TEST PASS\n");
    return 0;
}