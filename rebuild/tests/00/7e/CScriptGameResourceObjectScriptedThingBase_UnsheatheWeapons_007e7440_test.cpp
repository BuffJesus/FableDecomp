...
static void UnsheatheWeapons(Outer* self){ Inner* p=self->inner; if(p){ VFn fn=*reinterpret_cast<VFn*>(*reinterpret_cast<char**>(p)+0x64); fn(p); } }
int main(){ void* tbl[64]; for(int i=0;i<64;i++) tbl[i]=0;
 tbl[0x64/4]=(void*)tgt;
 Inner inr; inr.vtbl=(void**)tbl; Outer outr; outr.inner=&inr;
 UnsheatheWeapons(&outr);
 if(!g_hit){ std::printf("007e7440_TEST FAIL\n"); return 1;}
 std::printf("007e7440_TEST PASS\n"); return 0;}