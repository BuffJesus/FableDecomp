...
void* __fastcall CThingSwitch_CompositeScalarDeletingDestructor(void* self, int, unsigned int flags)
{
    ...
    AutoTinySecondMemberDestructor(static_cast<unsigned char*>(self) + 96);
...
int main()
{
    ...
    if (CThingSwitch_CompositeScalarDeletingDestructor(object, 0, 0) != object ||
        g_AutoTinyFirstMember != object + 108 ||
        g_AutoTinySecondMember != object + 96 ||
    ...
    if (g_AutoTinySequenceCount != 6 ||
        ...
        return 1;
    std::printf("AUTO_TINY_00e0e7b0_TEST PASS\n");
    return 0;
}