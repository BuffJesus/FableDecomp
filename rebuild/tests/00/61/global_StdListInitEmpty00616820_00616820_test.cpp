...
int main(){void* owner=(void*)1;if(Run(&owner)!=&owner||!owner)return 1;Node* node=(Node*)owner;if(node->next!=node||node->previous!=node)return 2;free(node);puts("LIST_SENTINEL_CTOR_00616820_PASS");return 0;}