#include <cstdio>

// ---- reproduce the source under test (self-contained) ----
class CCharString
{
public:
    CCharString(const char* str, int len);
    const char* data() const { return m_data; }
private:
    char* m_data;
};

// Local stand-in for CCharString::CCharString(const char*, int) @ 0x0099ebf0.
static const char* g_lastStr = 0;
static int g_lastLen = 0;
CCharString::CCharString(const char* str, int len)
{
    g_lastStr = str;
    g_lastLen = len;
    m_data = (char*)str;
}

class CCombatSequenceWhisperAttackCombo
{
public:
    CCharString GetName() const;
};

CCharString CCombatSequenceWhisperAttackCombo::GetName() const
{
    return CCharString((const char*)0x1278d20, -1);
}

int main()
{
    CCombatSequenceWhisperAttackCombo obj;
    CCharString s = obj.GetName();

    bool ok = true;
    if (g_lastStr != (const char*)0x1278d20) ok = false;
    if (g_lastLen != -1) ok = false;
    if (s.data() != (const char*)0x1278d20) ok = false;

    if (ok)
        printf("GETNAME_WHISPERATTACKCOMBO_OK\n");
    else
        printf("GETNAME_FAIL str=%p len=%d\n", (void*)g_lastStr, g_lastLen);

    return ok ? 0 : 1;
}