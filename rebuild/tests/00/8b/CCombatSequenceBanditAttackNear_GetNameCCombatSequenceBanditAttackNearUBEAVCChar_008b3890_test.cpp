...
int main()
{
	CCombatSequence_BanditAttackNear obj;
	CCharString s = obj.GetName();

	bool ok = true;
	if (g_lastStr != (const char*)0x012787dc) ok = false;
	if (g_lastLen != -1) ok = false;
	if (s.data() != (const char*)0x012787dc) ok = false;

	if (ok)
		printf("GETNAME_BANDITATTACKNEAR_OK\n");
	else
