...
int main()
{
    CCombatSequenceWhisperAttack360 obj;
    CCharString s = obj.GetName();

    bool ok = true;
    if (g_lastStr != (const char*)0x1278d48) ok = false;
    if (g_lastLen != -1) ok = false;
    if (s.data() != (const char*)0x1278d48) ok = false;

    if (ok)
        printf("GETNAME_WHISPERATTACK360_OK\n");
    else
