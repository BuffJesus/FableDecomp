...
{
    CCharString ret;
    CCharString_ctor(&ret, 0, (const char*)0x1278fb8, -1);
    return ret;
}

int main()
{
    CCombatSequence_WillPhysicalShieldNonMelee obj;
    CCharString s = obj.GetName();

    int ok = 1;
    if (s.m_data != (char*)0x1278fb8) ok = 0;
    if (g_last_str != (const char*)0x1278fb8) ok = 0;
