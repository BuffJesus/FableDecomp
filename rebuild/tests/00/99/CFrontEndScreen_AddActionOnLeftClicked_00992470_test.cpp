...
int main()
{
    ...
    ResetEvents();
    CFrontEndScreen_AddActionOnLeftClicked_00992470(&screen, 0, &incoming);
    ok &= Check(screen.left_click_action == old_action,
                "same control must preserve the existing action");
    ...
    ok &= Check(screen.left_click_action == old_action &&
                ...
                "two null controls must preserve the existing action");
    ...
    puts("PASS CFrontEndScreen::AddActionOnLeftClicked 0x00992470");
    return 0;
}
