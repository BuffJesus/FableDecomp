...
// Behaviour fixture for CActionDoCreatureAction::GetActionName @ 0x008fd8fd.
...
struct FableCharStringResult_008fd8fd
{
...
void* FableCharStringResult_008fd8fd::build(const char* text, int length)
{
    seen_text = text;
    seen_len = length;
    return this;
...
extern "C" FableCharStringResult_008fd8fd* FABLE_FASTCALL
FableGetActionName_008fd8fd(FableCharStringResult_008fd8fd* self, void*);

int main()
{
    FableCharStringResult_008fd8fd r;
    r.seen_text = 0;
    r.seen_len = 0;
    FableCharStringResult_008fd8fd* got = FableGetActionName_008fd8fd(&r, 0);
    if (got != &r) { std::printf("BAD_IDENTITY\n"); return 1; }
    if (r.seen_len != -1) { std::printf("BAD_LEN\n"); return 1; }
    if (r.seen_text == 0) { std::printf("BAD_TEXT\n"); return 1; }
    std::printf("GETACTIONNAME_008fd8fd_OK\n");
    return 0;
}