...
void* FableCharStringResult_008fe464::build(const char* text, int length)
{
    seen_text = text;
    seen_len = length;
    return this;
...
int main()
{
    FableCharStringResult_008fe464 r;
    r.seen_text = 0;
    r.seen_len = 0;
    FableCharStringResult_008fe464* got = FableGetActionName_008fe464(&r, 0);
    if (got != &r) { std::printf("BAD_IDENTITY\n"); return 1; }
    if (r.seen_len != -1) { std::printf("BAD_LEN\n"); return 1; }
    if (r.seen_text == 0) { std::printf("BAD_TEXT\n"); return 1; }
    std::printf("GETACTIONNAME_008fe464_OK\n");
    return 0;
}