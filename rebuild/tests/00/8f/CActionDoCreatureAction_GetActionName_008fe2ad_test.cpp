...
void* FableCharStringResult_008fe2ad::build(const char* text, int length)
{
    seen_text = text;
    seen_len = length;
    return this;
...
int main()
{
    FableCharStringResult_008fe2ad r;
    r.seen_text = 0;
    r.seen_len = 0;
    FableCharStringResult_008fe2ad* got = FableGetActionName_008fe2ad(&r, 0);
    if (got != &r) { std::printf("BAD_IDENTITY\n"); return 1; }
    if (r.seen_len != -1) { std::printf("BAD_LEN\n"); return 1; }
    if (r.seen_text == 0) { std::printf("BAD_TEXT\n"); return 1; }
    std::printf("GETACTIONNAME_008fe2ad_OK\n");
    return 0;
}