...
void* FableCharStringResult_008fe2e6::build(const char* text, int length)
{
    seen_text = text;
    seen_len = length;
    return this;
...
int main()
{
    FableCharStringResult_008fe2e6 r;
    r.seen_text = 0;
    r.seen_len = 0;
    FableCharStringResult_008fe2e6* got = FableGetActionName_008fe2e6(&r, 0);
    if (got != &r) { std::printf("BAD_IDENTITY\n"); return 1; }
    if (r.seen_len != -1) { std::printf("BAD_LEN\n"); return 1; }
    if (r.seen_text == 0) { std::printf("BAD_TEXT\n"); return 1; }
    std::printf("GETACTIONNAME_008fe2e6_OK\n");
    return 0;
}