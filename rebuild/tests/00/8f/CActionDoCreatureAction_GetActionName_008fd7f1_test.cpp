...
// Behaviour fixture for CActionDoCreatureAction::GetActionName @ 0x008fd7f1.
...
void* FableCharStringResult_008fd7f1::build(const char* text, int length)
{
    seen_text = text;
    seen_len = length;
    return this;
...
int main()
{
    FableCharStringResult_008fd7f1 r;
    r.seen_text = 0;
    r.seen_len = 0;
    FableCharStringResult_008fd7f1* got = FableGetActionName_008fd7f1(&r, 0);
    if (got != &r) { std::printf("BAD_IDENTITY\n"); return 1; }
    if (r.seen_len != -1) { std::printf("BAD_LEN\n"); return 1; }
    if (r.seen_text == 0) { std::printf("BAD_TEXT\n"); return 1; }
    std::printf("GETACTIONNAME_008fd7f1_OK\n");
    return 0;
}