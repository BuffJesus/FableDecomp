...
// Behaviour fixture for CActionDoCreatureAction::GetActionName @ 0x008fd89f.
...
struct FableCharStringResult_008fd89f
{
...
void* FableCharStringResult_008fd89f::build(const char* text, int length)
{
    seen_text = text;
    seen_len = length;
    return this;
...
extern "C" FableCharStringResult_008fd89f* FABLE_FASTCALL
FableGetActionName_008fd89f(FableCharStringResult_008fd89f* self, void*);

int main()
{
    FableCharStringResult_008fd89f r;
    r.seen_text = 0;
    r.seen_len = 0;
    FableCharStringResult_008fd89f* got = FableGetActionName_008fd89f(&r, 0);
    if (got != &r) { std::printf("BAD_IDENTITY\n"); return 1; }
    if (r.seen_len != -1) { std::printf("BAD_LEN\n"); return 1; }
    if (r.seen_text == 0) { std::printf("BAD_TEXT\n"); return 1; }
    std::printf("GETACTIONNAME_008fd89f_OK\n");
    return 0;
}