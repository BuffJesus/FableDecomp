...
void* FableCharStringResult_008fd420::build(const char* text, int length)
...
int main()
{
    FableCharStringResult_008fd420 r; r.seen_text = 0; r.seen_len = 0;
    FableCharStringResult_008fd420* got = FableGetActionName_008fd420(&r, 0);
    if (got != &r) { std::printf("BAD_IDENTITY\n"); return 1; }
    if (r.seen_len != -1) { std::printf("BAD_LEN\n"); return 1; }
    if (r.seen_text == 0) { std::printf("BAD_TEXT\n"); return 1; }
    std::printf("GETACTIONNAME_008fd420_OK\n"); return 0;
}