...
void FABLE_FASTCALL FableDestVal_008fd288_t::inner() { g_inner++; }
void FABLE_FASTCALL FableDestVal_008fd288_t::outer() { g_outer++; }
extern "C" void FABLE_FASTCALL FableDestVal_008fd288(FableDestVal_008fd288_t* self);
int main()
{
    char buf[0xc + 8];
    FableDestVal_008fd288((FableDestVal_008fd288_t*)buf);
    if (g_inner != 1) { std::printf("BAD_INNER\n"); return 1; }
    if (g_outer != 1) { std::printf("BAD_OUTER\n"); return 1; }
    std::printf("DESTVAL_008fd288_OK\n"); return 0;
}