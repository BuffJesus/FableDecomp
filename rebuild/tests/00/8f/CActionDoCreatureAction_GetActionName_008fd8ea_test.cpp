...
// Behaviour fixture for CActionDoCreatureAction::GetActionName @ 0x008fd8ea.
...
struct FableCharStringResult_008fd8ea
{
...
void* FableCharStringResult_008fd8ea::build(const char* text, int length)
{
    seen_text = text;
    seen_len = length;
    return this;
...
extern "C" FableCharStringResult_008fd8ea* FABLE_FASTCALL
FableGetActionName_008fd8ea(FableCharStringResult_008fd8ea* self, void*);

int main()
{
    FableCharStringResult_008fd8ea r;
    r.seen_text = 0;
    r.seen_len = 0;
    FableCharStringResult_008fd8ea* got = FableGetActionName_008fd8ea(&r, 0);
    if (got != &r) { std::printf("BAD_IDENTITY\n"); return 1; }
    if (r.seen_len != -1) { std::printf("BAD_LEN\n"); return 1; }
    if (r.seen_text == 0) { std::printf("BAD_TEXT\n"); return 1; }
    std::printf("GETACTIONNAME_008fd8ea_OK\n");
    return 0;
}