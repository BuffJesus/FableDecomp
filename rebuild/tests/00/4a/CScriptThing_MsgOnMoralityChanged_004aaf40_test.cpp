...
bool CScriptThing::MsgOnMoralityChanged(int a0){ CScriptThingInner *p=m_impl; if(!p) return false; return p->m58(a0); }
int main(){
    CScriptThing t; t.m_impl=0;
    if(t.MsgOnMoralityChanged(0)){std::printf("BADNULL\n");return 1;}
    CScriptThingInner impl; CScriptThingInner *pi = &impl; t.m_impl=pi;
    if(!t.MsgOnMoralityChanged(1)){std::printf("BADFWD\n");return 1;}
    std::printf("MSG_4aaf40_OK\n"); return 0;
}