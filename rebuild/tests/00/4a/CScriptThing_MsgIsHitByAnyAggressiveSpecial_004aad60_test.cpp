...
bool CScriptThing::MsgIsHitByAnyAggressiveSpecial(int a0){ CScriptThingInner *p=m_impl; if(!p) return false; return p->m43(a0); }
int main(){
    CScriptThing t; t.m_impl=0;
    if(t.MsgIsHitByAnyAggressiveSpecial(0)){std::printf("BADNULL\n");return 1;}
    CScriptThingInner impl; CScriptThingInner *pi = &impl; t.m_impl=pi;
    if(!t.MsgIsHitByAnyAggressiveSpecial(1)){std::printf("BADFWD\n");return 1;}
    std::printf("MSG_4aad60_OK\n"); return 0;
}