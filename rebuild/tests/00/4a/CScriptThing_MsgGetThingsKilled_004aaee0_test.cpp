...
bool CScriptThing::MsgGetThingsKilled(int a0){ CScriptThingInner *p=m_impl; if(!p) return false; return p->m55(a0); }
int main(){
    CScriptThing t; t.m_impl=0;
    if(t.MsgGetThingsKilled(0)){std::printf("BADNULL\n");return 1;}
    CScriptThingInner impl; CScriptThingInner *pi = &impl; t.m_impl=pi;
    if(!t.MsgGetThingsKilled(1)){std::printf("BADFWD\n");return 1;}
    std::printf("MSG_4aaee0_OK\n"); return 0;
}