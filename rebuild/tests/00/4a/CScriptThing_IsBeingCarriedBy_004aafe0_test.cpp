...
struct CScriptThingInner {
  ...
  virtual bool m60(int a0){ return true; }
  virtual bool m61(int a0){ return true; }
  virtual bool m62(int a0){ return true; }
  virtual bool m63(int a0){ return true; }
};
struct CScriptThing { void *pad0; CScriptThingInner *m_impl; bool IsBeingCarriedBy(int a0); };
bool CScriptThing::IsBeingCarriedBy(int a0){ CScriptThingInner *p=m_impl; if(!p) return false; return p->m63(a0); }
int main(){
    CScriptThing t; t.m_impl=0;
    if(t.IsBeingCarriedBy(0)){std::printf("BADNULL\n");return 1;}
    CScriptThingInner impl; CScriptThingInner *pi = &impl; t.m_impl=pi;
    if(!t.IsBeingCarriedBy(1)){std::printf("BADFWD\n");return 1;}
    std::printf("MSG_4aafe0_OK\n"); return 0;
}