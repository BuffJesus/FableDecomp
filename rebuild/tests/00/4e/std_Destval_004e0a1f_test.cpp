...
void FABLE_FASTCALL FableDestVal_004e0a1f_t::inner() { g_inner++; }
void FABLE_FASTCALL FableDestVal_004e0a1f_t::outer() { g_outer++; }
extern "C" void FABLE_FASTCALL FableDestVal_004e0a1f(FableDestVal_004e0a1f_t* self);
int main()
{
    char buf[0x8 + 8];
    FableDestVal_004e0a1f_t* self = (FableDestVal_004e0a1f_t*)buf;
    FableDestVal_004e0a1f(self);
    if (g_inner != 1) { std::printf("BAD_INNER\n"); return 1; }
    if (g_outer != 1) { std::printf("BAD_OUTER\n"); return 1; }
    std::printf("DESTVAL_004e0a1f_OK\n");
    return 0;
}