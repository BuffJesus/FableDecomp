...
static void Helper_65f300(CActiveFile* self) { self->helper_called = 1; }
static void my_free(void* p) { g_freed++; g_last_free = p; }

static void OnReadFinished_65f300(CActiveFile* self)
{
    Helper_65f300(self);
    void* p = self->buf;
    if (p)
