...
// Behaviour fixture for CActionDoCreatureAction::GetActionName @ 0x004d4edc.
...
void* FableCharStringResult_004d4edc::build(const char* text, int length)
{
    seen_text = text;
    seen_len = length;
    return this;
...
int main()
{
    FableCharStringResult_004d4edc r;
    r.seen_text = 0;
    r.seen_len = 0;
    FableCharStringResult_004d4edc* got = FableGetActionName_004d4edc(&r, 0);
    if (got != &r) { std::printf("BAD_IDENTITY\n"); return 1; }
    if (r.seen_len != -1) { std::printf("BAD_LEN\n"); return 1; }
    if (r.seen_text == 0) { std::printf("BAD_TEXT\n"); return 1; }
    std::printf("GETACTIONNAME_004d4edc_OK\n");
    return 0;
}