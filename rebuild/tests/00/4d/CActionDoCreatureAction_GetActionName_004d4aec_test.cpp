...
// Behaviour fixture for CActionDoCreatureAction::GetActionName @ 0x004d4aec.
...
struct FableCharStringResult_004d4aec
{
...
void* FableCharStringResult_004d4aec::build(const char* text, int length)
{
    seen_text = text;
    seen_len = length;
    return this;
...
extern "C" FableCharStringResult_004d4aec* FABLE_FASTCALL
FableGetActionName_004d4aec(FableCharStringResult_004d4aec* self, void*);

int main()
{
    FableCharStringResult_004d4aec r;
    r.seen_text = 0;
    r.seen_len = 0;
    FableCharStringResult_004d4aec* got = FableGetActionName_004d4aec(&r, 0);
    if (got != &r) { std::printf("BAD_IDENTITY\n"); return 1; }
    if (r.seen_len != -1) { std::printf("BAD_LEN\n"); return 1; }
    if (r.seen_text == 0) { std::printf("BAD_TEXT\n"); return 1; }
    std::printf("GETACTIONNAME_004d4aec_OK\n");
    return 0;
}