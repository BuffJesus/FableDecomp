...
// Behaviour fixture for CActionDoCreatureAction::GetActionName @ 0x004d5bee.
...
struct FableCharStringResult_004d5bee
{
...
void* FableCharStringResult_004d5bee::build(const char* text, int length)
{
    seen_text = text;
    seen_len = length;
    return this;
...
extern "C" FableCharStringResult_004d5bee* FABLE_FASTCALL
FableGetActionName_004d5bee(FableCharStringResult_004d5bee* self, void*);

int main()
{
    FableCharStringResult_004d5bee r;
    r.seen_text = 0;
    r.seen_len = 0;
    FableCharStringResult_004d5bee* got = FableGetActionName_004d5bee(&r, 0);
    if (got != &r) { std::printf("BAD_IDENTITY\n"); return 1; }
    if (r.seen_len != -1) { std::printf("BAD_LEN\n"); return 1; }
    if (r.seen_text == 0) { std::printf("BAD_TEXT\n"); return 1; }
    std::printf("GETACTIONNAME_004d5bee_OK\n");
    return 0;
}