...
// Behaviour fixture for CActionDoCreatureAction::GetActionName @ 0x004d3eb6.
...
struct FableCharStringResult_004d3eb6
{
...
void* FableCharStringResult_004d3eb6::build(const char* text, int length)
{
    seen_text = text;
    seen_len = length;
    return this;
...
extern "C" FableCharStringResult_004d3eb6* FABLE_FASTCALL
FableGetActionName_004d3eb6(FableCharStringResult_004d3eb6* self, void*);

int main()
{
    FableCharStringResult_004d3eb6 r;
    r.seen_text = 0;
    r.seen_len = 0;
    FableCharStringResult_004d3eb6* got = FableGetActionName_004d3eb6(&r, 0);
    if (got != &r) { std::printf("BAD_IDENTITY\n"); return 1; }
    if (r.seen_len != -1) { std::printf("BAD_LEN\n"); return 1; }
    if (r.seen_text == 0) { std::printf("BAD_TEXT\n"); return 1; }
    std::printf("GETACTIONNAME_004d3eb6_OK\n");
    return 0;
}