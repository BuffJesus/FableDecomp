...
// Behaviour fixture for CActionDoCreatureAction::GetActionName @ 0x004d2d4c.
...
struct FableCharStringResult_004d2d4c
{
...
void* FableCharStringResult_004d2d4c::build(const char* text, int length)
{
    seen_text = text;
    seen_len = length;
    return this;
...
extern "C" FableCharStringResult_004d2d4c* FABLE_FASTCALL
FableGetActionName_004d2d4c(FableCharStringResult_004d2d4c* self, void*);

int main()
{
    FableCharStringResult_004d2d4c r;
    r.seen_text = 0;
    r.seen_len = 0;
    FableCharStringResult_004d2d4c* got = FableGetActionName_004d2d4c(&r, 0);
    if (got != &r) { std::printf("BAD_IDENTITY\n"); return 1; }
    if (r.seen_len != -1) { std::printf("BAD_LEN\n"); return 1; }
    if (r.seen_text == 0) { std::printf("BAD_TEXT\n"); return 1; }
    std::printf("GETACTIONNAME_004d2d4c_OK\n");
    return 0;
}