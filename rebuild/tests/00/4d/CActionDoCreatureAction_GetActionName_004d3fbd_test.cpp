...
// Behaviour fixture for CActionDoCreatureAction::GetActionName @ 0x004d3fbd.
...
struct FableCharStringResult_004d3fbd
{
...
void* FableCharStringResult_004d3fbd::build(const char* text, int length)
{
    seen_text = text;
    seen_len = length;
    return this;
...
extern "C" FableCharStringResult_004d3fbd* FABLE_FASTCALL
FableGetActionName_004d3fbd(FableCharStringResult_004d3fbd* self, void*);

int main()
{
    FableCharStringResult_004d3fbd r;
    r.seen_text = 0;
    r.seen_len = 0;
    FableCharStringResult_004d3fbd* got = FableGetActionName_004d3fbd(&r, 0);
    if (got != &r) { std::printf("BAD_IDENTITY\n"); return 1; }
    if (r.seen_len != -1) { std::printf("BAD_LEN\n"); return 1; }
    if (r.seen_text == 0) { std::printf("BAD_TEXT\n"); return 1; }
    std::printf("GETACTIONNAME_004d3fbd_OK\n");
    return 0;
}