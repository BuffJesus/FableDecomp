...
// Behaviour fixture for CActionDoCreatureAction::GetActionName @ 0x004d3cae.
...
struct FableCharStringResult_004d3cae
{
...
void* FableCharStringResult_004d3cae::build(const char* text, int length)
{
    seen_text = text;
    seen_len = length;
    return this;
...
extern "C" FableCharStringResult_004d3cae* FABLE_FASTCALL
FableGetActionName_004d3cae(FableCharStringResult_004d3cae* self, void*);

int main()
{
    FableCharStringResult_004d3cae r;
    r.seen_text = 0;
    r.seen_len = 0;
    FableCharStringResult_004d3cae* got = FableGetActionName_004d3cae(&r, 0);
    if (got != &r) { std::printf("BAD_IDENTITY\n"); return 1; }
    if (r.seen_len != -1) { std::printf("BAD_LEN\n"); return 1; }
    if (r.seen_text == 0) { std::printf("BAD_TEXT\n"); return 1; }
    std::printf("GETACTIONNAME_004d3cae_OK\n");
    return 0;
}