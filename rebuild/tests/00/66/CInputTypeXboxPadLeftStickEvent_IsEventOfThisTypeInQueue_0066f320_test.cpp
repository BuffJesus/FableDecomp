...
static int RunCase(long index, unsigned char result)
{
    ...
    ok &= Check(
        CInputTypeXboxPadLeftStickEvent_IsEventOfThisTypeInQueue_0066f320(
            0, 0, 0x77, index, &strength) == result,
    ...
    ok &= Check(g_query_kind == 0x11 && g_query_state == 0,
                "query must request the left-stick event kind and state");
...
int main()
{
    ...
    puts("PASS CInputTypeXboxPadLeftStickEvent::IsEventOfThisTypeInQueue 0x0066F320");
    return 0;
}
