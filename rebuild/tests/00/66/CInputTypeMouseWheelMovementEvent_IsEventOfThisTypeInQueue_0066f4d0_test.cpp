...
static int RunCase(long index, unsigned char result)
{
    ...
    ok &= Check(
        CInputTypeMouseWheelMovementEvent_IsEventOfThisTypeInQueue_0066f4d0(
            0, 0, 0x33, index, &strength) == result,
    ...
    ok &= Check(g_query_kind == 0x0e && g_query_state == 0,
                "query must request mouse-wheel movement and state zero");
...
int main()
{
    ...
    puts("PASS CInputTypeMouseWheelMovementEvent::IsEventOfThisTypeInQueue 0x0066F4D0");
    return 0;
}
