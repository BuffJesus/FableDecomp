...
int main()
{
    ...
    ResetEvents();
    CFrontEndScreen_AddActionOnLeftClicked_00669280(&screen, 0, &incoming);
    ...
    ok &= Check(screen.left_click_control == &old_control,
                "same control must preserve existing control");
    ok &= Check(old_control.references == 3,
                "same control must preserve reference count");
    ...
    ok &= Check(screen.left_click_control == 0,
                "two null controls must remain null");
    ok &= Check(g_release_count == 0 && g_free_count == 0,
                "two null controls must not release");
    ...
    ok &= Check(screen.left_click_action == new_action,
                "new action must be assigned");
    ok &= Check(screen.left_click_control == &new_control,
                "new control must be assigned");
    ok &= Check(new_control.references == 6,
                "new control reference count must increment");
    ok &= Check(g_release_count == 0 && g_free_count == 0,
    ...
    ok &= Check(old_control.references == 1,
                "shared old control reference count must decrement");
    ok &= Check(new_control.references == 8,
                "replacement control reference count must increment");
    ok &= Check(screen.left_click_action == new_action &&
                screen.left_click_control == &new_control,
                "replacement counted action must be stored");
    ok &= Check(g_release_count == 0 && g_free_count == 0,
    ...
    ok &= Check(old_control.references == 0,
                "final old control reference must reach zero");
    ok &= Check(g_release_count == 1 &&
                g_released_payload == old_control.payload,
                "final old control must invoke its payload release");
    ok &= Check(g_free_count == 1 &&
                g_freed_control == &old_control,
    ...
    puts("PASS CFrontEndScreen::AddActionOnLeftClicked");
    return 0;
}
