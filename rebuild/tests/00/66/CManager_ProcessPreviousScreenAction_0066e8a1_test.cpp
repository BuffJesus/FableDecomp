...
static unsigned char __fastcall ProcessPrevious(Screen* screen, void*)
{
...
int main()
{
    ...
    vtable.process_previous = ProcessPrevious;
    ...
    ResetCalls(0);
    CManager_ProcessPreviousScreenAction_0066e8a1(&manager, 0, 3, 3);
    ok &= Check(manager.current_action == 1,
                "non-boundary call must decrement the current action");
    ...
    ok &= Check(manager.current_action == 0,
                "boundary call must clamp the current action to zero");
    ok &= Check(scroll_indices[0] == 2 &&
                scroll_indices[1] == 1 &&
                scroll_indices[2] == 0,
                "boundary call must decrement and wrap every scroll index");
    ...
    manager.current_action = 0;
    ResetCalls(1);
    CManager_ProcessPreviousScreenAction_0066e8a1(&manager, 0, 1, 3);
    ok &= Check(manager.current_action == 0,
                "single action must remain at index zero");
    ok &= Check(scroll_indices[0] == 1,
                "rejected screen must decrement again until accepted");
    ...
    manager.current_action = 0;
    ResetCalls(0);
    CManager_ProcessPreviousScreenAction_0066e8a1(&manager, 0, 0, 3);
    ok &= Check(manager.current_action == 0,
                "zero screen count must retain the zero sentinel");
    ok &= Check(scroll_indices[0] == 2 && g_call_count == 0,
                "zero screen count must skip the processing loop");

    if (!ok)
        return 1;

    puts("PASS CManager::ProcessPreviousScreenAction");
    return 0;
}
