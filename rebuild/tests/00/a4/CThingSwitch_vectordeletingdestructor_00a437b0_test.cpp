...
{
    AutoTinyFirstMemberDestructor(static_cast<unsigned char*>(self) + 36);
    AutoTinySecondMemberDestructor(static_cast<unsigned char*>(self) + 28);
...
int main()
{
    ...
    if (CThingSwitch_CompositeScalarDeletingDestructor(object, 0, 0) != object ||
        g_AutoTinyFirstMember != object + 36 ||
        g_AutoTinySecondMember != object + 28 ||
    ...
    if (g_AutoTinySequenceCount != 6 ||
        ...
        return 1;
    std::printf("AUTO_TINY_00a437b0_TEST PASS\n");
    return 0;
}