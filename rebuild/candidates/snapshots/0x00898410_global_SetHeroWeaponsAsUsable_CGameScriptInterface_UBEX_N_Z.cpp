...
{
public:
    void __thiscall SetWeaponsAsUsable(bool isUsable);
};
...
{
public:
    void __thiscall SetHeroWeaponsAsUsable(std::uint32_t param3Raw) const;
};
...
struct CGameScriptInterface_SetHeroWeaponsAsUsable_SelfOverlay
{
...
static_assert(
    offsetof(
        CGameScriptInterface_SetHeroWeaponsAsUsable_SelfOverlay,
...
struct CGameScriptInterface_SetHeroWeaponsAsUsable_TCInterfaceEntryOverlay
{
...
static_assert(
    offsetof(
        CGameScriptInterface_SetHeroWeaponsAsUsable_TCInterfaceEntryOverlay,
        m_Key) == 0x00);
static_assert(
    offsetof(
        CGameScriptInterface_SetHeroWeaponsAsUsable_TCInterfaceEntryOverlay,
...
struct CGameScriptInterface_SetHeroWeaponsAsUsable_TargetOverlay
{
...
static_assert(
    offsetof(
        CGameScriptInterface_SetHeroWeaponsAsUsable_TargetOverlay,
        m_Flags24) == 0x24);
static_assert(
    offsetof(
        CGameScriptInterface_SetHeroWeaponsAsUsable_TargetOverlay,
...
struct CGameScriptInterface_SetHeroWeaponsAsUsable_HeroOverlay
{
    std::byte m_Pad00[0x0E];
    std::uint8_t m_Field0E; // 0x0E
};

static_assert(
    offsetof(
        CGameScriptInterface_SetHeroWeaponsAsUsable_HeroOverlay,
        m_Field0E) == 0x0E);

struct CGameScriptInterface_SetHeroWeaponsAsUsable_CategoryOverlay
{
    std::byte m_Pad00[0x24];
    void* m_Field24; // 0x24
};
...
static_assert(
    offsetof(
        CGameScriptInterface_SetHeroWeaponsAsUsable_CategoryOverlay,
        m_Field24) == 0x24);

struct CGameScriptInterface_SetHeroWeaponsAsUsable_CategoryStateOverlay
{
    void** m_pVftable;       // 0x00
    std::int32_t m_RefCount; // 0x04
    std::byte m_Pad08[0x68];
    std::uint8_t m_Field70; // 0x70
};

static_assert(
    offsetof(
        CGameScriptInterface_SetHeroWeaponsAsUsable_CategoryStateOverlay,
        m_RefCount) == 0x04);
static_assert(
    offsetof(
        CGameScriptInterface_SetHeroWeaponsAsUsable_CategoryStateOverlay,
...
using Fn_00487DC0 =
    CGameScriptInterface_SetHeroWeaponsAsUsable_TargetOverlay* (__thiscall*)(void*);
using RawGetInterfaceFn = void* (__thiscall*)(void*);
using RawReleaseFn =
    void (__thiscall*)(CGameScriptInterface_SetHeroWeaponsAsUsable_CategoryStateOverlay*);
...
void __thiscall CGameScriptInterface::SetHeroWeaponsAsUsable(std::uint32_t param3Raw) const
{
    const auto* const self =
        reinterpret_cast<
            const CGameScriptInterface_SetHeroWeaponsAsUsable_SelfOverlay*>(this);
    auto* const target =
        CALL_00487DC0(CALL_00449970(self->m_Field14));

    if ((target != nullptr) && ((target->m_Flags91 & 0x01) == 0))
    {
        auto* pHero = reinterpret_cast<CTCHero*>(static_cast<std::uintptr_t>(param3Raw));
        ...
        if ((target->m_Flags24 & 0x00000200) != 0)
        {
            ...
            pHero = reinterpret_cast<CTCHero*>(pEntry->m_pInterface);
        }

        const bool isLowByteZero =
            static_cast<std::uint8_t>(param3Raw) == static_cast<std::uint8_t>(0);

        reinterpret_cast<CGameScriptInterface_SetHeroWeaponsAsUsable_HeroOverlay*>(pHero)
            ->m_Field0E = static_cast<std::uint8_t>(isLowByteZero);

        if (isLowByteZero)
        {
            pHero->SetWeaponsAsUsable(false);
        }
        else
        {
            ...
            if (pCategoryState != nullptr)
            {
                ...
                if (static_cast<char>(pCategoryState->m_Field70) == '\0')
                {
                    pHero->SetWeaponsAsUsable(true);
                }
