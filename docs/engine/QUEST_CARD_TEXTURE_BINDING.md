...
| **Shared re-skin (all cards of a class)** | ✓ via data | Replace texture id 5892/5894/5896 in textures.big (`tools/texture_build.py`, `docs/formats/TEXTURE_WRITER.md`). Up to 3 distinct looks by repointing the 3 CUIDef `GraphicIndex` values (needs a frontend.bin CUIDef **writer** — verify `forge defs encode` supports the UI class). |
...
1. **Custom texture** into textures.big — READY today (`tools/texture_build.py`,
   docs/formats/TEXTURE_WRITER.md; match the DXT format/dims of 5892 — TODO: confirm Info tail/mips/WxH).
...
- `docs/engine/QUEST_CARD_SYSTEM.md`, `docs/formats/TEXTURE_WRITER.md`, `docs/formats/FRONTEND_FORMAT.md`.
