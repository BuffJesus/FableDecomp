...
- [ ] Repair the latest ForgeFSE wrapper batch before promotion. Ten structural-PASS wrappers
  ...
  implausible BSim-derived action helper identities. The exact list is in `docs/journal/HANDOFF_ARCHIVE.md`.
- [ ] Rebuild the latest Quest-wrapper review tail before promotion. `TakeObjectFromHero`,
  ...
  ABI evidence is recorded in `docs/journal/HANDOFF_ARCHIVE.md` and `rebuild/corrections/function_overrides.tsv`.
...
The boundary and subsystem readiness criteria are in `docs/ARCHITECTURE.md`.
...
`docs/journal/HANDOFF_ARCHIVE.md` and this section, then inspect the current Wave 3 outputs under
...
`docs/pipeline/DECOMP_ACCELERATORS.md`.
...
2. **Mass-name & type** (Phases 1–3) via the labelling loop (`docs/pipeline/METHODOLOGY.md`): RTTI classes →
