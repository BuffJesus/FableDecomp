...
Full spec: `docs/formats/TEXTBIG_FORMAT.md`. **Three independent sources agree** (raw bytes + EgoCore
...
(933 already-bound functions). Full spec: `docs/scripts/SCRIPT_BINDINGS_CANDIDATES.md`.
...
Full details in `docs/engine/NAVMESH_RE.md` ("SUBDIVISION ALGORITHM — RECOVERED"); tool in
...
Full evidence: `docs/formats/SAVE_SIG_ENFORCEMENT.md`; logs `ghidra_out/decomp_save_sig_load1..4.log`.
...
Full spec: `docs/formats/QST_FORMAT.md`; parser: `tools/parse_qst.py` (parse/roundtrip/diff).
...
Full evidence: `docs/engine/QUEST_VM_RE.md`; artifacts `ghidra_out/quest_*`.
...
field table: `docs/formats/TEXTURE_WRITER.md`.
...
design and validation: `docs/formats/MESH_COMPOSE.md`. Key facts (EgoCore `MeshCompiler.h`
...
`docs/formats/DEF_LOAD_CONTRACT.md`. Bytes from retail `Fable.exe` + `data/CompiledDefs/{game,names}.bin`.
...
Full write-up: `docs/modding/README.md#coop_revival`. Byte-solid (opcode-proven): enable gate = `[CNetworkClient+0x2662]`
...
identical. Full evidence: `docs/scripts/STATUEMASTER_LUA_PORT_AUDIT.md`; synthesized
report and explicitly speculative restoration design:
`docs/scripts/STATUEMASTER_DEEP_RESEARCH_REPORT.md`.
...
Full implementation handoff: `docs/engine/STATUE_WEAPON_AUGMENTATION_RE.md`.
