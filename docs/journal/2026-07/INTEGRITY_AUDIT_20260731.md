...
| 3,4,20 | high/med | `tools/render_fable_frontend_subscreens.py`, `docs/journal/HANDOFF_ARCHIVE.md` | `PIXEL_IDENTICAL`/"live visual parity" = agreement with authored Python bake, not retail pixels | **PENDING** — rename metric → `bake_self_consistent` / `AUTHORED_BAKE_MATCH`; qualify HANDOFF "parity" wording. |
...
| 21,22 | med | `docs/journal/HANDOFF_ARCHIVE.md`, frontend tests | "live visual parity" / self-consistency tests; File-Info panel approximate; SPOOKY bg baked not retail | **PENDING** — label as authored/behavior, not parity. |
