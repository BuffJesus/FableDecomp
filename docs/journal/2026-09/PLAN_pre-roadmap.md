...
`docs/pipeline/FULL_DECOMP.md` for the dedicated strategy; this is the destination the phases below climb toward.
...
6. **Document** — append to `docs/journal/FINDINGS_LOG.md` + `docs/engine/SYSTEMS_ANALYSIS.md`; commit the label TSV
...
- [ ] **Deliverable:** `docs/engine/SYSTEMS_ANALYSIS.md` skeleton — every major subsystem located and named
...
### Phase 4 — FULL source-level decompilation (the primary goal — full detail in `docs/pipeline/FULL_DECOMP.md`)
...
4. Keep `docs/journal/HANDOFF_ARCHIVE.md` current — it is the resume point.

See `docs/BUILDING.md` for exact commands, `docs/modding/README.md#ecosystem` for the full community-tool survey,
and `docs/pipeline/METHODOLOGY.md` for the Ghidra-scripting mechanics inherited from Fable2RE.
