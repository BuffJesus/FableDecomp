...
- Parity crawl: resume from `work/crawl_batch155` — `next_smallest.py` -> `trim_overcapture.py` -> `trim_tailjmp.py` -> author -> `verify_and_land.py`; iterate a DIFFER with `work/crawl_batch154/probe.py`. Ledger `tools/decomp_pipeline/crawl/gen_tried.txt`.
