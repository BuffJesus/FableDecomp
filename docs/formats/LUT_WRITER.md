...
*Authored 2026-07-19. Companion to `docs/formats/LUT_AUDIOBANK_FORMAT.md` (the READ/extraction spec)
...
`docs/formats/LUT_AUDIOBANK_FORMAT.md` §3/§5 flagged the tail `LHAudioBankLookupTable` "hash bucket array"
...
Toolpack codec is the other option). Codec semantics + validation: `docs/formats/LUG_AUDIO_FORMAT.md`
...
- **Adding NEW indices** — SHIPPED 2026-07-20: `rebuild(b, add=(N, record))` appends one clip at
  ...
  291 empty gaps is still untested (append-only for now). See docs/formats/DIALOGUE_PIPELINE.md.
