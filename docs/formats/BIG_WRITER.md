...
Extends the proven read + byte-exact round-trip work (`docs/formats/BIG_ROUNDTRIP.md`,
`docs/formats/BIG_FAMILY_FORMATS.md`) into a controlled WRITE prototype:
...
- **`CRC` (TOC +0x14) — passthrough verbatim.** Per `docs/formats/BIG_ROUNDTRIP.md` §7 the TOC CRC is a
...
- **Authoring a brand-new format CLASS**, or changing an entry's format so its class fingerprint
  ...
  string, which is **not present in the `.big` bytes** (see `docs/formats/BIG_ROUNDTRIP.md` §7.3/§7.5).
