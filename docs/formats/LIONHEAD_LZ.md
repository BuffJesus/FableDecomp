...
This closes the one open gap in `docs/formats/BIG_TEXTURE_FORMAT.md` §4.2. Deliverable
...
- **`graphics.big` mesh (MBANK) payloads: NO — not LZO-framed.** Their payloads begin
  ...
  leading bytes are not an LZO stream. Mesh data is stored per `docs/formats/BIG_MESH_FORMAT.md`
