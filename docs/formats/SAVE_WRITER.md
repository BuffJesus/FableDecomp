...
**Companion spec:** `docs/formats/SAVEGAME_FORMAT.md` (the format crack; this doc is the write-side proof).
...
- **Semantic** validity of an edited field value: the gates prove byte-level correctness, not that a
  ...
  in `docs/formats/SAVE_HEADER_MAP.md`) is the remaining "which field means what" work — orthogonal to this
