...
*2026-07-20. Status: **SHIPPED & VALIDATED.** Write side of `docs/formats/BIG_ANIM_FORMAT.md` §9
(read side) and `docs/engine/DEMON_DOOR_FACE.md` (lipsync chain). Answer key: EgoCore
...
5. **Speech mouth-sync.** Author a `LipSync` (viseme dictionary → the target creature's
  ...
  Pair with a `.lut` voice line (docs/formats/LUT_WRITER.md) sharing the entry id/name scheme.
...
- ~~Blender bridge: exporter from Blender actions → `Track` pools~~ **SHIPPED 2026-07-20**
  — see §7 and `docs/modding/README.md#blender_addon` §1.5 (`io_scene_fable/anim_ops.py`, both
...
`export_action_to_image`). Full detail in `docs/modding/README.md#blender_addon` §1.5; the
