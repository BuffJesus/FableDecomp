...
- [ ] Typed shared headers: 2026-09-07 `--all-trusted` = 969 compiling headers (244 quarantined); automatic passes plus the parity-gated manual tail have retyped 532 landed TUs onto PDB-named headers (47 manual conversions across six checkpoints). Next lever = the live skip reasons in `rebuild/backlog/retype_log.tsv`; the manual pass has reduced VTABLE_TYPED from 37 to 13 and VIRTUAL_THIS from 54 to 52 — `tools/decomp_pipeline/`
...
- [x] 2026-09-07 automatic and manual parity-gated retyping applied to 532 landed TUs (see Current focus for the skip-reason worklist)
