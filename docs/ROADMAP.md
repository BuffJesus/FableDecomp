...
- [ ] Typed shared headers: 2026-09-07 `--all-trusted --compile-check` = 957 compiling headers (257 quarantined); automatic passes plus the parity-gated manual tail have retyped 638 landed TUs onto PDB-named headers (149 manual conversions across thirty-one checkpoints). `VTABLE_TYPED` (37→0), `VIRTUAL_THIS` (54→0), non-source-failure `PARITY_CHANGED` (7→0), and `BEHAV_FAIL` (8→0) are closed. The source-failure parity lane is 43→11 after twelve exact `CScriptThing` proxies, the six-function copy/transfer subset, three troll effect-name, two expression, and three `CGameScriptInterface` recoveries, two other recoveries, and four current reclassifications; the ten-entry `AMBIG_ALIGN` review recovered two, reclassified six, and isolated two unnamed donor-layout dependencies — `tools/decomp_pipeline/`
