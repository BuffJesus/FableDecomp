...
- [ ] Typed shared headers: 2026-09-07 `--all-trusted` = 969 compiling headers (244 quarantined); automatic passes plus the parity-gated manual tail have retyped 545 landed TUs onto PDB-named headers (60 manual conversions across eight checkpoints). `VTABLE_TYPED` is closed (37→0); next lever is the 52 `VIRTUAL_THIS` cases in the live `rebuild/backlog/retype_log.tsv` — `tools/decomp_pipeline/`
...
- [x] 2026-09-07 automatic and manual parity-gated retyping applied to 545 landed TUs; `VTABLE_TYPED` tail closed (see Current focus for the next skip-reason lane)
