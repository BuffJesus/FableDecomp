...
- [ ] Typed shared headers: 2026-09-07 `--all-trusted --compile-check` = 957 compiling headers (257 quarantined); automatic passes plus the parity-gated manual tail have retyped 590 landed TUs onto PDB-named headers (105 manual conversions across twenty checkpoints). `VTABLE_TYPED` is closed (37→0); `VIRTUAL_THIS` is down from 54 to 7 in the live `rebuild/backlog/retype_log.tsv` — `tools/decomp_pipeline/`
...
- [x] 2026-09-07 automatic and manual parity-gated retyping applied to 590 landed TUs; `VTABLE_TYPED` tail closed (see Current focus for the next skip-reason lane)
