...
- [ ] Typed shared headers: 2026-09-07 `--all-trusted` = 969 compiling headers (244 quarantined); automatic passes plus the parity-gated manual tail have retyped 581 landed TUs onto PDB-named headers (96 manual conversions across fifteen checkpoints). `VTABLE_TYPED` is closed (37→0); `VIRTUAL_THIS` is down from 54 to 16 in the live `rebuild/backlog/retype_log.tsv` — `tools/decomp_pipeline/`
...
- [x] 2026-09-07 automatic and manual parity-gated retyping applied to 581 landed TUs; `VTABLE_TYPED` tail closed (see Current focus for the next skip-reason lane)
